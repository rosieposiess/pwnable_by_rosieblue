...
void print_addr(char* buf) { //이 함수로 스택의 리턴 어드레스 구하기
    ...
    printf("\nEnter appropriate address and offsets which satisfy the condition below.\n");
    printf("---------------------------------------------------------------\n");
    printf("| *(long*)((char*)addr + off1) == *(long*)((char*)buf + off2) |\n");
    printf("---------------------------------------------------------------\n");
    
    printf("Address(addr): ");
    read(0, &addr, 8); //stdin주소

    printf("First Offset(off1): ");
    read(0, &off1, 2);

    printf("Second Offset(off2): ");
    read(0, &off2, 2);

    if (*(long*)((char*)addr + off1) == *(long*)((char*)buf + off2)) {
